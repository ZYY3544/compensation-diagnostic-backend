...
def generate_summary():
    ...
    try:
        ...
        prompt = (
            "你是 Sparky，刚完成了一轮 6 题的业务访谈。\n\n"
            "请生成一句过渡语，告诉用户你接下来要做的事情：\n"
            "- 你要对右边的访谈纪要做一遍整体检查和整理\n"
            "- 看看有没有遗漏、矛盾、或者需要补充确认的地方\n\n"
            "参考示例（不要照搬，用自己的话说）：\n"
            "「接下来我先把右边的访谈纪要整体理一遍，检查有没有遗漏、矛盾，或者需要补充确认的地方。」\n\n"
            "要求：\n"
            "- 只需要 1-2 句话，40-80 字\n"
            "- 语气自然，像顾问跟客户说「我先把纪要理一遍」\n"
            "- 不要做任何总结或分析（总结会在后面单独做）\n"
            "- 不要用加粗标记\n"
            "- 用中文\n"
            "- 直接输出纯文本，不要 JSON"
        )
        ...
        return jsonify({
            'summary': '接下来我先把右边的访谈纪要整体理一遍，检查有没有遗漏、矛盾，或者需要补充确认的地方。'
        })
...
def review_interview():
    ...
    try:
        ...
        prompt = (
            ...
            "【第二件事 — reply】分两段：\n"
            "  第 1 段：简短说你在纪要上做了什么调整（比如"
            "\"我把战略方向那块的格式整理了一下，核心职能补了几个加粗标记\"），1-2 句话。\n"
            "  第 2 段：用开放式的引导语，问用户除了刚才聊的六个话题之外，有没有其他想补充的信息。"
            "角度是「充盈访谈内容」而不是「检查纪要有没有问题」。"
            "这段引导语用 **加粗** 标记，让它在视觉上突出。"
        ...
        return jsonify({
            'updates': [],
            'reply': '纪要整理下来挺完整的，格式也没什么需要调整的。除了刚才聊的这些，你还有没有其他想补充的信息？如果没有的话，点击右边的「确认纪要 →」，我会生成一段关键提炼发现。'
        })
